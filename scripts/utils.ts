import { AccountWalletWithSecretKey, createPXEClient, PXE, waitForPXE, createAztecNodeClient, AztecNode } from "@aztec/aztec.js";
...
export const mintTokensToWallet = async (tokenContract: any, ownerWallet: AccountWalletWithSecretKey, recipientAddress: any, amount: number): Promise<void> => {
  try {
    Logger.step(`Minting ${amount} tokens to wallet`);
    const mintTx = await (await TokenContract.at(tokenContract.address, ownerWallet)).methods.mint_to_public(recipientAddress, amount).send().wait();
...
export const checkTokenPublicBalance = async (tokenContract: any, walletAddress: any, walletName: string = "Wallet"): Promise<bigint> => {
  try {
